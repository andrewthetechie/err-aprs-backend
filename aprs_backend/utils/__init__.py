...
class ErrbotObjectStoreMixin(APRSDObjectStoreMixing):
    ...
    def _save_filename(self):
        ...
        filename = f"{filename_prefix}{self.__class__.__name__}{filename_suffix}{extension}"
    ...
    def save(self):
        ...
        if len(self) > 0:
            log.info(
                "%s::Saving %d entries to disk at %s", self.__class.__name__, len(self), self.save_location)
            with open(self.save_filename, "wb+") as fp:
                pickle.dump(self._dump(), fp)
        else:
            log.debug(
                "%s::Nothing to save, flushing old save file '%s'", self.__class__.__name__, self.save_filename)
            self.flush()
    ...
    def load(self):
        ...
        if os.path.exists(self.save_filename):
            try:
                with open(self.save_filename, "rb") as fp:
                    ...
                    if raw:
                        ...
                        log.debug(
                            "%s::No data to load", self.__class__.__name__)
            ...
            log.debug(
                "%s::No save file found", self.__class__.__name__)
